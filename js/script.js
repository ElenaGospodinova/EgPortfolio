...
const sendEmail = (e) => {
    ...
    const form = document.getElementById('contact-form-data'); // Get the form element
    const btn = document.querySelector('.contact_btn'); // Get the submit button
    const spinner = btn.querySelector('.fa-spinner'); // Get the spinner element
    const resultDiv = document.getElementById('result'); // Get the result message div

    // Show loading state (spinner and disable button)
    spinner.classList.remove('d-none'); // Show the spinner
    btn.setAttribute('disabled', true); // Disable the button while submitting
    btn.style.backgroundColor = 'green'; // Change the button color to green

    // Send email using EmailJS
    emailjs.sendForm('service_sxlo62s', 'template_jx6omqo', form)
        .then((result) => {
        ...
        .finally(() => {
            // Hide the spinner and enable the button after submission is complete
            spinner.classList.add('d-none'); // Hide the spinner
            btn.removeAttribute('disabled'); // Enable the button
            btn.style.backgroundColor = ''; // Reset the button color to the original state
        });
}; 